if(room == room_map) {
	if(!global.is_loading){
		...
		switch (load_state) {
		    ...
		    // ---------------------------------------------------------
		    // Previous event destroyed instances.
		    // By the time we reach this Step, they are actually gone.
		    // ---------------------------------------------------------
			case 1:
				load_state = 2
				break;
		    ...
		    case 2:
		        ...
		        load_create_instances();
				instance_deactivate_all(true)
		        break;


		    // ---------------------------------------------------------
		    // Instances were created during the previous Step.
		    // Now all references can safely be resolved.
		    // ---------------------------------------------------------

		    case 3:
				instance_activate_all()
		        load_resolve_references();
		        break;
		}
	}
}
