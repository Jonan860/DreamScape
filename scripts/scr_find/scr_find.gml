...
function scr_find_damage_spell_target(damage_spell){
...
function scr_find_spell_target_tile(spell) {
	...
	for(i = 0; i < array_length(varTilesWithinRange); i++) {
		var var_goodness = 0;
		var varTilesInAoe = scr_get_tiles_within_range(spell.getAoe(), varTilesWithinRange[i])
			
		for(j = 0; j < array_length(varTilesInAoe); j++) {
			var varunit = array_first(varTilesInAoe[j].occupants[? ALTITUDES.ground])
			if(!is_undefined(varunit) and spell.targetable(varunit)) {
				var_goodness += spell.evaluateGoodness(varunit)
			}
		}
		if(var_goodness > var_optimal_goodness) {
			var_optimalTile = varTilesWithinRange[i]
			var_optimal_goodness = var_goodness
		}
	}
	return var_optimalTile
}
