function scr_creep_lord_hunt() {
	if(owner=global.creep_lord and phase="hunt" and action_bar>=min(attack_cost,movement_cost)) ////kanske ska begränsas till friendlies only
	{
	...
	if(ds_list_empty(path)) ///maybe should be ds_list_empty?
	{
	phase="idle" 
	action_bar=0
	target=noone
