function scr_right_pressed_enemy(var_ground_unit,clicked_tile,var_selected_unit){	
	if(var_selected_unit == global.ida) {
		with(var_ground_unit) {
			var already_freezed = freezed.applied
		}
		if(cursor_sprite == spr_freeze_cursor and !already_freezed) {
			if(global.ida.freeze.lvl > 0) {
				...
				var mana_cost = global.freeze_struct.mana_cost_list[|global.ida.freeze.lvl - 1]
				if(global.ida.freeze.cooldown_current == 0 and scr_get_distance(global.tile_selected, clicked_tile) == 1 and global.ida.mana >= mana_cost) {
