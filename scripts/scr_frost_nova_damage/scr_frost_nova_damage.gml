function scr_frost_nova_damage() {
	var damage_nova=ds_list_find_value(list_of_frost_nova_damage_per_lvl,spell_q_lvl-1)
	...
	{
		for(i=0;i<ds_list_size(var_ground_list);i+=1)
		{
	...
	{
		for(i=0;i<ds_list_size(var_air_list);i+=1)
		{
		var	var_air=ds_list_find_value(var_air_list,i)
	...
	{
		for(i=0;i<ds_list_size(var_invisible_list);i+=1)
		{
		var var_invisible=ds_list_find_value(var_invisible_list,i)
	...
	var var_invisible_list=ds_map_find_value(occupants,"invisible")
		...
		if(!ds_list_empty(var_ground_list))
	{
		for(j=0;j<ds_list_size(var_ground_list);j+=1)
		{
	var		var_ground=ds_list_find_value(var_ground_list,j)
		if(var_ground.object_index!=obj_crystal and !object_is_ancestor(var_ground.object_index,obj_building))
			{
	if(scr_is_enemies(var_ground,other))
	{
	...
	if(!ds_list_empty(var_air_list))
	{
		for(k=0;k<ds_list_size(var_air_list);k+=1)
		{
		var	var_air=ds_list_find_value(var_air_list,k)
		if(var_air.object_index!=obj_crystal  and !object_is_ancestor(var_air.object_index,obj_building))
			{
	if(scr_is_enemies(var_air,other))
	{
	...
	if(!ds_list_empty(var_invisible_list))
	{
		for(n=0;n<ds_list_size(var_invisible_list);n+=1)
		{
			var_invisible=ds_list_find_value(var_invisible_list,n)
		if(var_invisible.object_index!=obj_crystal  and !object_is_ancestor(var_invisible.object_index,obj_building))
			{
	if(scr_is_enemies(var_invisible,other))
	{
