function scr_attack_target_golden_dragon() {
	...
	var x_distance=target_tile.x-x
	var y_distance=target_tile.y-y
	x=x+1.5*sprite_get_width(spr_hexagon_pink)*x_distance/(sqrt(power(x_distance,2)+power(y_distance,2)))///x=0.3*x1+0.7*x2
	y=y+1.5*sprite_get_width(spr_hexagon_pink)*y_distance/(sqrt(power(x_distance,2)+power(y_distance,2)))///y=0.3*y1+0.7*y2

	}
	//light_beam.altitude=target.altitude
	}
