function scr_attack_hp_reduction(var_perputrator, var_victim) {
	...
	if(object_is_ancestor(var_perputrator.object_index, obj_unit)) {
		...
		if(var_victim.object_index == obj_crypt_lord) {
			if(var_victim.spell_w_lvl > 0 and var_perputrator.range == 1) {
				var return_damage_fraction = ds_list_find_value(var_victim.list_of_returned_damage_from_spiked_carapacelvl, var_victim.spell_w_lvl - 1)
