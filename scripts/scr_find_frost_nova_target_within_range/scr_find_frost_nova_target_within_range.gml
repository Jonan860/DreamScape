function scr_find_frost_nova_target_within_range() {
	...
	loopTilesStart
		if(scr_get_distance(self, other.tile) <= other.frost_nova.range) {
			...
			var var_air_unit_list = occupants[? ALTITUDES.air]
			var var_goodness_of_frost_nova = 0
			if(!array_equals(var_ground_unit_list, [])) {
				for(var i = 0; i < array_length(var_ground_unit_list); i++) {
			...
			if(!array_equals(var_air_unit_list, [])) {
				for(var i = 0; i < array_length(var_air_unit_list); i++) {
			...
			for(var i = 0; i <= array_length(list_of_neighbours) - 1; i++) {
				...
				if(!array_equals(var_ground_unit_list, [])) {
					for(var j = 0; j < array_length(var_ground_unit_list); j++) {
				...
				if(!array_equals(var_air_unit_list, [])) {
					for(var k = 0; k < array_length(var_air_unit_list); k++) {
