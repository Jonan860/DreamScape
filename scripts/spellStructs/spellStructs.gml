...
function spellToAoe(spell) {
	switch(spell) {
		case SPELLS.silence : return [2, 2, 3]
		default : return 0
	}
}
...
function evaluateSilence(_unit) {
	return _unit.mana > 50 ? _unit.max_mana : 0
}
...
function createSpellSub(spellEnum) constructor {
	...
	aoe = spellToAoe(spellEnum)
	targetable = spellToTargetable(spellEnum)
	...
	shouldPerform = function() {
		return autocast and canPerform() and shouldPerformLocal()
	}
	shouldRightPerform = function() {
		return canPerform() and shouldRightPerformLocal()
	}
	...
	canPerform = function() {
		return spellCanPerform() and canPerformLocal()
	}
	canRecruit = function() {
...
function createSpell(spellEnum, _letter) {
	...
	with(spell) {
		...
		getAoe = function() {
			return is_array(aoe) ? aoe[max(lvl - 1, 0)] : aoe
		}
		getSpellHealth = function() {
			return is_array(spellHealth) ? spellHealth[max(lvl - 1, 0)] : spellHealth
		}
		...
		canPerformLocal = spellToCanPerformLocal(spellEnum)
		...
		leastAcceptableGoodness = spellToLeastAcceptableGoodness(spellEnum)
		letter = _letter
